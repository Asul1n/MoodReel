...
| DEEPSEEK_API_KEY / DEEPSEEK_ENABLED | 空 | DeepSeek 大模型情感分析（中文通道默认 + 评论批量补标，`/analyze/zh`、`/analyze/backfill`） |
| BAIDU_API_KEY / BAIDU_SECRET_KEY / BAIDU_ENABLED | 空 | 百度情感倾向分析 API（备选回退） |
